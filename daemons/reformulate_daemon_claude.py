"""
Claude API-powered reformulation daemon.
Sends the selected text to Anthropic's Claude API and gets back a complete
reformulation in a single request (no local model, no GPU/MPS needed).

Requirements: pip install anthropic
Auth: export ANTHROPIC_API_KEY=sk-ant-... (or `ant auth login`)
"""
import json
import logging
import os
...
log.info("Using Claude API (%s). Hotkey active: Ctrl+Shift+Space", MODEL_NAME)
...
def reformulate(sentence, max_tokens=600):
    ...
    try:
        ...
        retry_after = e.response.headers.get("retry-after", "a few") if e.response else "a few"
        log.error("Rate limited by Claude API, retry after %s seconds", retry_after)
        return []
    except anthropic.APIStatusError as e:
        log.error("Claude API error (%s): %s", e.status_code, e.message)
        return []
    except anthropic.APIConnectionError:
        log.error("Could not reach the Claude API — check your internet connection")
        return []
